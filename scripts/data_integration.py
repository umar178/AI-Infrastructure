...
def main():
    ...
    # 3. Add district information to health and education data
    print("3. Adding district information...")
    try:
        health_enriched = health_df.merge(
            lookup_df[['name_clean', 'District_City']],
            left_on='city_clean',
            right_on='name_clean',
            how='left'  # Keep all records even if no match found
        )

        education_enriched = education_df.merge(
            lookup_df[['name_clean', 'District_City']],
            left_on='city_clean',
            right_on='name_clean',
            how='left'
